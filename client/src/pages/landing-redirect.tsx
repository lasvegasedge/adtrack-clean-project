...
// Create a component that conditionally renders the landing page or redirects to dashboard
export default function LandingRedirect() {
  ...
  // Fetch pricing data from the system
  // Make sure pricing data is up-to-date when component mounts
  useEffect(() => {
    const getPricingFromAPI = async () => {
      try {
        // Fetch pricing data from the public API with a cache busting parameter
        const response = await fetch(`/api/pricing-public?_=${Date.now()}`);
        if (!response.ok) {
        ...
        if (data && Array.isArray(data) && data.length > 0) {
          ...
          data.forEach(plan => {
            ...
            if (plan.name === "Pro Plan" && plan.price) {
              pro = plan.price;
            }
            if (plan.name === "Enterprise Plan" && plan.price) {
              premium = plan.price;
            }
          });
          ...
          console.log("Updated pricing from Admin Panel:", {
            basicPrice: basic,
            proPrice: pro,
            premiumPrice: premium
          });
    ...
    // Call the function to get pricing data
    getPricingFromAPI();
  }, []);
  ...
  // Directly render the landing page with inline HTML for simplicity
  return (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                <li><Link href="#" className="hover:text-white">ROI Tracking</Link></li>
                <li><Link href="#" className="hover:text-white">Competitor Analysis</Link></li>
                <li><Link href="#" className="hover:text-white">AI Recommendations</Link></li>
                <li><Link href="#" className="hover:text-white">Marketing Insights</Link></li>
              </ul>
            </div>
            <div>
